...
│   │   └── models/    # keeper spine + auth (one module per cluster; enums in enums.py; account.py + gathering.py since CK-12, event.py gone)
│   ├── alembic/       # async-template env; 0001 = Phase 1 spine, 0002 = auth + household ladder, 0003 = token ToS version, 0004 = profile columns (IANA timezone, updated_at), 0005 = anonymized_at (deletion = anonymization, never cascade), 0006 = email_change_requests, 0007 = webauthn credentials + challenges, 0008 = keeper spine structural half (accounts, gatherings + occurrences, invitations; events/event_series dropped)
│   ├── tests/         # pytest + httpx ASGI suite (test_auth.py, test_profile.py, test_account_deletion.py, test_email_change.py, test_sessions.py, test_passkeys.py — SoftPasskey software authenticator; test_schema_keeper.py — CK-12 spine shape; conftest owns the test DB)
